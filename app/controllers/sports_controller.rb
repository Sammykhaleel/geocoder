...
if params[:search].present?
    @sports = Sport.near(params[:search], 50) 
    
    else
     @sports = Sport.all
    end
      @hash = Gmaps4rails.build_markers(@sports) do |sport, marker|
      marker.lat sport.latitude
      marker.lng sport.longitude
      marker.infowindow sport.site
    end    end
  ...
  private
    ...
    def sport_params
      params.require(:sport).permit(:address, :latitude, :longitude, :site)
    end
end
