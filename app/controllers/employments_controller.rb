class EmploymentsController < ApplicationController
  ...
  def index
   if params[:search].present?
    @employments = Employment.near(params[:search], 50) 
    
    else
     @employments = Employment.all
    end
      @hash = Gmaps4rails.build_markers(@employments) do |employment, marker|
      marker.lat employment.latitude
      marker.lng employment.longitude
      marker.infowindow employment.site
    end  
  end
  ...
  private
    ...
    def employment_params
      params.require(:employment).permit(:address, :latitude, :longitude, :site)
    end
end
