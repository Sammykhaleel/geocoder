...
if params[:search].present?
    @legals = Legal.near(params[:search], 50) 
    
    else
     @legals = Legal.all
    end
      @hash = Gmaps4rails.build_markers(@legals) do |legal, marker|
      marker.lat legal.latitude
      marker.lng legal.longitude
      marker.infowindow legal.site
    end    end
  ...
  private
    ...
    def legal_params
      params.require(:legal).permit(:address, :latitude, :longitude, :site)
    end
end
